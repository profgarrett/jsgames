...
*/
const function_arguments: IStringIndexJsonObject = {
	'left': [ 'String', 'Number'],
	'right': [ 'String', 'Number'],
	'floor': ['Number'],
	'round': ['Number', 'Number'],
...
function build_reference(ref: string) {
	const json = {
			'type': 'reference_'+ref,
			'message0': '%1 %2', 
			'args0': [
...
function build_value(value: any, accept_right_input=true) {
...
function build_value_number_input(accept_right_input: any) {
	const json = accept_right_input
		?
		{
			'type': 'block_type',
...
function build_value_string_input(accept_right_input: any) {
	const json = accept_right_input
		?
		{
			'type': 'block_type',
...
const symbol_to_word: IStringIndexJsonObject = {
	'<': 'lt',
	'>': 'gt',
	'<=': 'le',
	'>=': 'ge',
...
function build_symbol(symbol: string) {
...
function build_function(f_name: string) {
...
const add_check_if_not_any = (json: any, check: any) => {
...
function build_function1(f_name: string, arg_type0: any){
...
function build_function2(f_name: string, arg_type0: any, arg_type1: any){
...
function build_function3(f_name: string, arg_type0: any, arg_type1: any, arg_type2: any){
...
function build_block(block_title: string, f: any) {
...
*/
const toolbox_sort_order: IStringIndexJsonObject = {
	equal: 0,
	functions: 1,
	symbols: 2,
	references: 3,
...
const get_toolbox_item_sort = (s: string) => {
...
*/
function get_toolbox_xml( page: any ) {
	// Template-aware text.
	const t = (s: string) => {
	...
	source.forEach( (rule: any) => {
		
		if (rule.has === 'functions' ) {
			rule.args.map( (value: any) => t(value)).forEach( (value: any) => 
	...
	// @ts-ignore
	toolbox.sort( (a: any, b: any) => get_toolbox_item_sort(a) > get_toolbox_item_sort(b) );
