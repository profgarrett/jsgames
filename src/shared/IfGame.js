...
const { has_references, has_symbols, has_no_values, has_values } = require('./has');
...
const __get_feedback = (that: PageType): Array<string> => {
	let response = '';
	let responses = [];
	let has = {
		'references': has_references,
		'no_values': has_no_values,
		'symbols': has_symbols,
		'values': has_values
	};
	...
	if(that.correct === true) responses.push( 'Correct answer!');
	if(that.correct === false) responses.push( 'Incorrect answer.');
...
function common_schema(): Object {
	...
	return {
		...
		solution_feedback: { type: 'Array', initialize: (a) => isDef(a) && isArray(a) ? a : [] },
		feedback: { type: 'Array', initialize: (a) => isDef(a) && isArray(a) ? a : [] },
...
class IfPageTextSchema extends Schema {
	...
	// We never have any feedback for text schemas.
	get_feedback(): ?string {
		return null;
	}
...
class IfPageChoiceSchema extends Schema {
	...
	// Find the first non-null feedback and return it.  If none found, then use defaults.
	get_feedback(): ?string {
		// $FlowFixMe
		return __get_feedback(this);
	}
...
class IfPageFormulaSchema extends Schema {
	...
	*/
	__clean_parse_formula( dirty_formula: string ): string {
		let d = /\d/;
		let formula = dirty_formula.toUpperCase().trim();
		//let D = new RegExp('\D');

		// Issue 1: Uppercase, as true doesn't work, but TRUE does.
