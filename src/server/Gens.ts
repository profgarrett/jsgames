...
const LinearGen = (seed: number, pages: typeof IfPageBaseSchema[], gen: GenType): any => {
	...
	while(result === null) {
		...
		total++;
		// @ts-ignore
		if( typeof last_page !== 'undefined' && last_page !== null && last_page.correct === true) {
			correct++;
...
*/
const ShuffleGenUntilLimit = (seed: number, pages: typeof IfPageBaseSchema[], gen: GenType): any => {
...
*/
const AdaptiveGen = (seed: number, pages: typeof IfPageBaseSchema[], gen: AdaptiveGenType): any => {
	let result = null;
	let pre_test_gen_pages: any[] = []; // pages.slice() will be run in the while looop to populate this.
	let consumed_pages: any[] = [];
...
*/
const runGen_copy = (seed: number, param_pages: typeof IfPageBaseSchema[], gen: string): any => {
...
// Use for external reference.
const runGen = runGen_copy;

// Nocopy is used internally to run gens.
// If a gen needs to consume a page, it shouldn't be making a copy.
const runGen_nocopy = (seed: number, pages: typeof IfPageBaseSchema[], gen: string): any => {
	const translate: IStringIndexJsonObject = {
		'UntilGen': UntilGen,
...
export {
	runGen, UntilGen, ShuffleGen, LinearGen, AdaptiveGen, ShuffleGenUntilLimit
}