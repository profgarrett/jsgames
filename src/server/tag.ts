...
import { parseFeedback } from './../shared/parseFeedback';
...
const ENTRY_TESTS = [
	{
		...
		if: (solution_f: string, client_f: string, page: typeof IfPageFormulaSchema) => {
			const parsed  = parseFeedback(client_f);
			const references = parsed.filter( has => has.has === 'references' );
