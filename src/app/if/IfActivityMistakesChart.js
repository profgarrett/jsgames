...
export default class IfActivityMistakesChart extends React.Component<PropsType> {

	build_data( levels: Array<LevelType> ): any {
...
*/
		...
		tutorial_pages.forEach( (Ap: Array<PageType>, i: number) => {
			max_revisions = Ap.reduce( (max: number, p: PageType): number => Math.max(max, p.history.length), 0);
			divs.push(<div key={i}><IfActivityLevelChartPage pages={Ap} width={600} height={max_revisions*height_per_revision} /></div>);
		});
		
		test_pages.forEach( (Ap: Array<PageType>, i: number) => {
			max_revisions = Ap.reduce( (max: number, p: PageType): number => Math.max(max, p.history.length), 0);
			divs.push(<div key={i}><IfActivityLevelChartPage pages={Ap} width={600} height={max_revisions*height_per_revision} /></div>);
		});
...
class IfActivityLevelChartPage extends React.Component<PagePropsType> {
	constructor(props: any) {
		super(props);
		(this: any).createChart = this.createChart.bind(this);
	}
	...
	createChart() {
		...
		const enhance_history = (p: PageType): PageType => {
		...
		const data = dirty_data.map( (p: PageType, i: number): any => 
		...
		const xScale = d3.scaleBand()
			.domain(data.map( (p: PageType, i: number): string => i ))
		...
		*/
		...
		chart.selectAll('rect')
			.attr('class', 'bar')
			.attr('x', (p: PageType): string => xScale(p.i)+margin.l)
			.attr('y', (p: PageType): number => yScale(0)+margin.t)
			.attr('width', bar_width)
			.attr('height', (p: PageType): number => yScale(p.history.length))
		...
		chart.selectAll('rect')
			.on('mouseover', function(d) {
				...
				d.history.filter( h => h.show).forEach( (h: any, i: number) => {
					let t = 
